...
def compute_integration_percentage(target_info, alignment_dir, sample_name):
    ...
    for index, row in target_info_df.iterrows():
                ...
                for condition, cat_keys in rules:
                    if condition:
                        for key in cat_keys:
                            ...
                            if 'attL' in key:
                                if read.mapping_quality > 0: attL_records[umi] = seq_record
                                if has_indel_in_range(read,beacon_quant_lower,beacon_quant_upper):
                                    indel_counter += 1
                            elif 'attR' in key:
                                if read.mapping_quality > 0: attR_records[umi] = seq_record
                                if has_indel_in_range(read,beacon_quant_lower,beacon_quant_upper):
                                    indel_counter += 1
                            elif 'beacon' in key and 'AA' in row['id']:
                                if read.mapping_quality > 0: beacon_records[umi] = seq_record
                                if has_indel_in_range(read,beacon_quant_lower,beacon_quant_upper):
                                    indel_counter += 1
                            elif 'wt' in key:
        ...
        integration_dict[row['id']] = (
            ...
            indel_percent
        ) + tuple(row[key] for key in ['gene_name', 'gene_strand', 'gene_distance', 'same_strand', 'overlapping_feature', 'threat_tier','Cosmic Gene','Role in Cancer','Liver Expression (TPM)'])
