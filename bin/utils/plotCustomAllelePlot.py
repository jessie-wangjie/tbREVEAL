...
def main():
    ...
    parser.add_argument("-o", "--output_root", type=str, help="Plot output root (should not include '.pdf' or '.png')",
                        required=True)
    parser.add_argument("--min_freq", "--min_frequency_alleles_around_cut_to_plot", type=float,
    ...
    parser.add_argument("--plot_right", help="Number of bases to plot to the right of the cut site", type=int,
                        default=20)
    parser.add_argument("--plot_center",
                        help="Center of plot. If set, plots for guide RNAs will not be generated -- only a plot centered at this position will be plotted.",
                        type=int, default=None)
    parser.add_argument("-a", "--amplicon", type=str, help="Specify amplicon")
...
def get_row_around_cut_assymetrical(row, cut_point, plot_left, plot_right):
    ...
    return row['Aligned_Sequence'][cut_idx - plot_left + 1:cut_idx + plot_right + 1], row['Reference_Sequence'][
                                                                                      cut_idx - plot_left + 1:cut_idx + plot_right + 1], \
    ...
    if (cut_point + plot_right + 1 > len(ref1)):
        raise (BadParameterException(
            'The plotting window cannot extend past the end of the amplicon. Amplicon length is ' + str(
                len(ref1)) + ' but plot extends to ' + str(cut_point + plot_right + 1)))

    df_alleles_around_cut = pd.DataFrame(list(
        df_alleles.apply(lambda row: get_row_around_cut_assymetrical(row, cut_point, plot_left, plot_right),
                         axis=1).values),
        columns=['Aligned_Sequence', 'Reference_Sequence', 'Unedited', 'n_deleted',
...
def plot_alleles_tables_from_folder(crispresso_output_folder, fig_filename_root, plot_left=20, plot_right=20,
                                    MIN_FREQUENCY=None, MAX_N_ROWS=None, SAVE_ALSO_PNG=False, custom_colors=None,
                                    plot_cut_point=True, sgRNA_intervals=None, sgRNA_names=None, sgRNA_mismatches=None,
                                    plot_center=None, amplicon_name=None):
    """
    ...
    for ref_name in ref_names:
        ...
        if plot_center is not None:
            sgRNA_label = 'custom_plot'

            if amplicon_name is not None:
                ref_name = amplicon_name
            ...
            df_alleles_around_cut = get_dataframe_around_cut_assymetrical(
                df_alleles[df_alleles['Aligned_Reference_Names'] == ref_name], cut_point, plot_left, plot_right)
            ...
            fig_filename_name = os.path.join(fig_filename_root, ref_name) + "_" + sgRNA_label
            plot_alleles_table(ref_seq_around_cut, df_alleles=df_alleles_around_cut,
                               fig_filename_root=fig_filename_name, cut_point_ind=cut_point - new_sel_cols_start,
...
def plot_alleles_table(reference_seq, df_alleles, fig_filename_root, MIN_FREQUENCY=0.5, MAX_N_ROWS=100,
                       SAVE_ALSO_PNG=False, plot_cut_point=True, cut_point_ind=None, sgRNA_intervals=None,
                       sgRNA_names=None, sgRNA_mismatches=None, custom_colors=None, annotate_wildtype_allele='****', ):
    """
    ...
    plot_alleles_heatmap(reference_seq, fig_filename_root, X, annot, y_labels, insertion_dict, per_element_annot_kws,
                         SAVE_ALSO_PNG, plot_cut_point, cut_point_ind, sgRNA_intervals, sgRNA_names, sgRNA_mismatches,
                         custom_colors)


def plot_alleles_heatmap(reference_seq, fig_filename_root, X, annot, y_labels, insertion_dict, per_element_annot_kws,
                         SAVE_ALSO_PNG=False, plot_cut_point=True, cut_point_ind=None, sgRNA_intervals=None,
                         sgRNA_names=None, sgRNA_mismatches=None, custom_colors=None):
    """
    ...
    if N_ROWS < 1:
        ...
        plt.text(0.5, 0.5, 'No Alleles', horizontalalignment='center', verticalalignment='center',
    ...
    CRISPRessoPlot.custom_heatmap(ref_seq_hm, annot=ref_seq_annot_hm, annot_kws={'size': 16}, cmap=cmap, fmt='s',
                                  ax=ax_hm_ref, vmin=0, vmax=5, square=True)
    CRISPRessoPlot.custom_heatmap(X, annot=np.array(annot), annot_kws={'size': 16}, cmap=cmap, fmt='s', ax=ax_hm,
                                  vmin=0, vmax=5, square=True, per_element_annot_kws=per_element_annot_kws)
    ...
    if sgRNA_intervals and len(sgRNA_intervals) > 0:
        ...
        CRISPRessoPlot.add_sgRNA_to_ax(ax_hm_ref, sgRNA_intervals, sgRNA_y_start=this_sgRNA_y_start,
                                       sgRNA_y_height=this_sgRNA_y_height, amp_len=plot_nuc_len, font_size='small',
                                       clip_on=False, sgRNA_names=sgRNA_names, sgRNA_mismatches=sgRNA_mismatches,
                                       x_offset=0, label_at_zero=True, sgRNA_rows=sgRNA_rows)
    ...
    for idx, lss in insertion_dict.items():
        for ls in lss:
            ax_hm.add_patch(
                patches.Rectangle((ls[0], N_ROWS - idx - 1), ls[1] - ls[0], 1, linewidth=3, edgecolor='r', fill=False))
    ...
    sns.set_context(
        rc={'axes.facecolor': 'white', 'lines.markeredgewidth': 1, 'mathtext.fontset': 'stix', 'text.usetex': True,
